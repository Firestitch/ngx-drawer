...
export class FsDrawerRouteComponent implements OnInit, OnDestroy {
  ...
  public async openDrawer(): Promise<void> {
    ...
    if (drawerConfig?.component) {
      if (drawerConfig.component instanceof Promise) {
        this._drawer = await this._openLazyDrawer(drawerConfig);
      } else {
        this._drawer = this._openDrawer(drawerConfig);
      }
  ...
  private async _openLazyDrawer(drawerConfig: IFsDrawerRouteConfig): Promise<DrawerRef<unknown, unknown>> {
